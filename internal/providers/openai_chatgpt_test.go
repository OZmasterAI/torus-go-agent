...
// ── Finding #4: encrypted reasoning is captured and round-tripped ──

func TestParseResponsesSSEReasoningRoundTrip(t *testing.T) {
	t.Parallel()

	sse := "data: {\"type\":\"response.reasoning_summary_text.delta\",\"delta\":\"thinking hard\"}\n\n" +
		"data: {\"type\":\"response.output_item.done\",\"item\":{\"type\":\"reasoning\",\"id\":\"rs_123\",\"encrypted_content\":\"ENC_ABC\"}}\n\n" +
		"data: {\"type\":\"response.output_text.delta\",\"delta\":\"the answer\"}\n\n" + sseCompleted

	_, final := collectResponsesSSE(sse)
	if final == nil {
		t.Fatal("no final message assembled")
	}

	// Capture side: the thinking block carries the encrypted reasoning state.
	var thinking *tp.ContentBlock
	for i := range final.Content {
		if final.Content[i].Type == "thinking" {
			thinking = &final.Content[i]
		}
	}
	if thinking == nil {
		t.Fatal("no thinking block captured")
	}
	if thinking.EncryptedContent != "ENC_ABC" {
		t.Errorf("EncryptedContent = %q, want %q", thinking.EncryptedContent, "ENC_ABC")
	}
	if thinking.ReasoningID != "rs_123" {
		t.Errorf("ReasoningID = %q, want %q", thinking.ReasoningID, "rs_123")
	}

	// Re-emit side: buildResponsesInput echoes a reasoning item, positioned
	// before the assistant message it preceded.
	items := buildResponsesInput([]tp.Message{final.Message})
	reasoningIdx, messageIdx := -1, -1
	for i, it := range items {
		switch it.Type {
		case "reasoning":
			reasoningIdx = i
			if it.EncryptedContent != "ENC_ABC" {
				t.Errorf("reasoning item EncryptedContent = %q, want %q", it.EncryptedContent, "ENC_ABC")
			}
			if it.ID != "rs_123" {
				t.Errorf("reasoning item ID = %q, want %q", it.ID, "rs_123")
			}
		case "message":
			if messageIdx == -1 {
				messageIdx = i
	...
	if reasoningIdx == -1 {
		t.Fatal("buildResponsesInput did not emit a reasoning item")
	}
	if messageIdx != -1 && reasoningIdx > messageIdx {
		t.Errorf("reasoning item (idx %d) must precede the message item (idx %d)", reasoningIdx, messageIdx)
	}

	// Confirm the round-tripped item marshals with the wire field names.
	raw, err := json.Marshal(items[reasoningIdx])
	if err != nil {
		t.Fatalf("marshal reasoning item: %v", err)
	}
	if !strings.Contains(string(raw), "\"encrypted_content\":\"ENC_ABC\"") {
		t.Errorf("marshaled reasoning item missing encrypted_content: %s", string(raw))
	}
}

// A thinking block without captured encrypted content must NOT emit a reasoning
// item — this preserves the pre-fix behavior for providers/turns that never set
// encrypted reasoning state.
func TestBuildResponsesInputSkipsReasoningWithoutEncryptedContent(t *testing.T) {
	t.Parallel()

	msg := tp.Message{
		Role: tp.RoleAssistant,
		Content: []tp.ContentBlock{
			{Type: "thinking", Text: "just a plain summary"},
			{Type: "text", Text: "hello"},
		},
	}
	items := buildResponsesInput([]tp.Message{msg})
	for _, it := range items {
		if it.Type == "reasoning" {
			t.Fatalf("unexpected reasoning item emitted for a thinking block without encrypted content: %+v", it)
		}
	}
}
